 ...
 */
const getAuthInfo = () => {
  const { APPLE_ID: appleId, APPLE_ID_PASSWORD: appleIdPassword, API_KEY_ID: appleApiKey, API_KEY_ISSUER_ID: appleApiIssuer, TEAM_SHORT_NAME: teamShortName } = process.env;

  if (!appleId && !appleIdPassword && !appleApiKey && !appleApiIssuer) {
    throw new Error("Authentication environment variables for notarization are missing. Either APPLE_ID and " + "APPLE_ID_PASSWORD, or API_KEY_ID and API_KEY_ISSUER_ID must be defined.");
  }

  if ((appleId || appleIdPassword) && (appleApiKey || appleApiIssuer)) {
    throw new Error("Should only provide either Apple ID or API key environment variables.");
  }
  ...
  if ((appleApiKey && !appleApiIssuer) || (!appleApiKey && appleApiIssuer)) {
    throw new Error("One of API_KEY_ID and API_KEY_ISSUER_ID environment variables is missing for notarization.");
  }
...
module.exports = async (params) => {
  ...
  if (!isEnvTrue(process.env.CSC_FOR_PULL_REQUEST) && ((process.env.CIRCLE_BRANCH && process.env.CIRCLE_BRANCH !== "master") || (process.env.TRAVIS_BRANCH && process.env.TRAVIS_BRANCH !== "master"))) {
    return;
  }
